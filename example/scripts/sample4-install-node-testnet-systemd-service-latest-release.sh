...
systemctl enable "$snowblossom_home/systemd/snowblossom-node-testnet.service"
# (re)start
systemctl restart snowblossom-node-testnet.service

echo "Done!"
echo "You can manage the systemd service normally with:"
echo "manage:   systemctl {start|stop|restart} snowblossom-node-testnet.service"
echo "logs  :   journalctl -n 100 -f -u snowblossom-node-testnet.service"
